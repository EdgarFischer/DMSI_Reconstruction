...
#### Definie Model path
if GT_Data == "FullRank":
    model_save_dir = f"../saved_models/Ynet"+DOMAIN+"/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
elif GT_Data == "LowRank":
    model_save_dir = f"../saved_models/Ynet/"+DOMAIN+"/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
    
#### Define log directory path
if GT_Data == "FullRank":
    log_dir = f"../log_files/Ynet/"+DOMAIN+"/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
elif GT_Data == "LowRank":
    log_dir = f"../log_files/Ynet/"+DOMAIN+"/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
...
#### Reshaping !
Undersampled_Data, Ground_Truth, mask_extended, GM_mask_expanded = Undersampled_Data.transpose(reshape_vector), Ground_Truth.transpose(reshape_vector), mask_extended.transpose(reshape_vector), GM_mask_expanded.transpose(reshape_vector)
...
val_mask_GM,  test_mask_GM = reshape_for_pytorch(Mask_val_GM, grouped_time_steps), reshape_for_pytorch(Mask_test_GM, grouped_time_steps)
...
val_dataset = TensorDatasetWithGM(val_data, val_labels, val_mask, val_mask_GM)
#train_dataset_unaugmented = TensorDataset(train_data, train_labels, train_mask)
test_dataset = TensorDatasetWithGM(test_data, test_labels, test_mask, test_mask_GM)
...
aug_data, aug_labels, aug_mask, aug_mask_gm = NN_input_train, ground_truth_train, Train_Mask, Train_Mask_GM        #transform_6d_data_labels_mask(NN_input_train, ground_truth_train, Train_Mask)
...
aug_data, aug_labels, aug_mask, aug_mask_gm = aug_data.transpose(reshape_vector), aug_labels.transpose(reshape_vector), aug_mask.transpose(reshape_vector), aug_mask_gm.transpose(reshape_vector)
...
train_data, train_labels, train_mask, train_mask_gm  = reshape_for_pytorch(aug_data, grouped_time_steps), reshape_for_pytorch(aug_labels, grouped_time_steps), reshape_for_pytorch(aug_mask, grouped_time_steps), reshape_for_pytorch(aug_mask_gm, grouped_time_steps)

train_dataset = TensorDatasetWithGM(train_data, train_labels, train_mask, train_mask_gm)
...
for epoch in range(start_epoch, num_epochs):
    # ---- Train for one epoch ----
    _ = train_one_epoch_ynet(model, optimizer, loss_fn, train_loader, device=device)
    
    # ---- Evaluate on TRAIN data ----
    avg_loss_train = validate_model_ynet(model, loss_fn, train_loader_unaug, device=device)
    
    # ---- Evaluate on VALIDATION data ----
    avg_loss_val = validate_model_ynet(model, loss_fn, val_loader, device=device)
    
    # ---- Evaluate on TEST data ----
    avg_loss_test = validate_model_ynet(model, loss_fn, test_loader, device=device)
