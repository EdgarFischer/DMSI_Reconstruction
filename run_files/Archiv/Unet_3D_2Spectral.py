...
from data_augmentation_Fit import *
...
ground_truth_path = "../data/Ground_Truth/Spectra_Fit_After_LR8/Spectral_Fit.npy"
    
#### Assemble saving path ####

#### Definie Model path
if GT_Data == "FullRank":
    model_save_dir = f"../saved_models/UNet_3D_2Spectral/"+DOMAIN+"/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
elif GT_Data == "LowRank":
    model_save_dir = f"../saved_models/UNet_3D_2Spectral/"+DOMAIN+"/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
    
#### Define log directory path
if GT_Data == "FullRank":
    log_dir = f"../log_files/UNet_3D_2Spectral/"+DOMAIN+"/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask
elif GT_Data == "LowRank":
    log_dir = f"../log_files/UNet_3D_2Spectral/"+DOMAIN+"/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask

#### Define Input Data path
undersampled_data_path = "../data/Undersampled_Data/"+Undersampling+f'/AF_{AF}/'+Sampling_Mask+'/data.npy'

#### load data!
Ground_Truth = np.load(ground_truth_path)[...,:160,:,:]
Undersampled_Data = np.zeros_like(Ground_Truth)

helper = np.load(undersampled_data_path)


if Low_Rank_Input:
    for i in range(0,6):
        helper[...,i] = low_rank(helper[...,i], 8)
#         if GT_Data == "LowRank":
#             Ground_Truth[...,i] = low_rank(Ground_Truth[...,i], 8)
    
Undersampled_Data[...,:96,:,:] = helper
...
mask_extended = np.broadcast_to(mask_expanded, (22, 22, 21, 160, 8, 6))
...
#### reshape for pytorch ####
val_data, val_labels = reshape_for_pytorch(NN_input_val, grouped_time_steps), reshape_for_pytorch(ground_truth_val, grouped_time_steps, is_mask=True)
...
test_data, test_labels = reshape_for_pytorch(NN_input_test, grouped_time_steps), reshape_for_pytorch(ground_truth_test, grouped_time_steps, is_mask=True)

val_mask,  test_mask = reshape_for_pytorch(Mask_val, grouped_time_steps, is_mask=True), reshape_for_pytorch(Mask_test, grouped_time_steps, is_mask=True)
...
aug_data, aug_labels, aug_mask = transform_6d_data_labels_mask(NN_input_train, np.real(ground_truth_train), Train_Mask)
...
train_data, train_labels, train_mask  = reshape_for_pytorch(aug_data, grouped_time_steps), reshape_for_pytorch(aug_labels, grouped_time_steps, is_mask=True), reshape_for_pytorch(aug_mask, grouped_time_steps, is_mask=True)
...
train_labels_unaug = reshape_for_pytorch(train_labels_unaug, grouped_time_steps, is_mask=True)
train_mask_unaug = reshape_for_pytorch(train_mask_unaug, grouped_time_steps, is_mask=True)
