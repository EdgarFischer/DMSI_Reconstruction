...
from Naive_CNN_3D_masked_abs import *
...
#### Definie Model path
if GT_Data == "FullRank":
    model_save_dir = f"../saved_models/AbsMasked_Naive_CNN_3D/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask+f'/{num_convs}Layer'
elif GT_Data == "LowRank":
    model_save_dir = f"../saved_models/AbsMasked_Naive_CNN_3D/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask+f'/{num_convs}Layer'
    
#### Define log directory path
if GT_Data == "FullRank":
    log_dir = f"../log_files/AbsMasked_Naive_CNN_3D/Full2Full/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask+f'/{num_convs}Layer'
elif GT_Data == "LowRank":
    log_dir = f"../log_files/AbsMasked_Naive_CNN_3D/Low2Low/"+Undersampling+f'/AF_{AF}/'+f'Truncate_t_{trancuate_t}/'+Sampling_Mask+f'/{num_convs}Layer'

#### Define Input Data path
undersampled_data_path = "../data/Undersampled_Data/"+Undersampling+f'/AF_{AF}/'+Sampling_Mask+'/data.npy'

#### load data!
Ground_Truth = np.abs(np.load(ground_truth_path))
Undersampled_Data = np.abs(np.load(undersampled_data_path))
...
train_data, train_labels  = train_data[:, 0:1, :, :, :], train_labels[:, 0:1, :, :, :]
test_data, test_labels = test_data[:, 0:1, :, :, :], test_labels[:, 0:1, :, :, :]
train_mask, test_mask = train_mask[:, 0:1, :, :, :], test_mask[:, 0:1, :, :, :]

#### prepare data for NN #####

# Create TensorDataset instances
augment = RandomAugment3D(rotation_range=0.2, shift_pixels=1,
                          apply_phase=False, apply_rotation=True, apply_shift=True)
