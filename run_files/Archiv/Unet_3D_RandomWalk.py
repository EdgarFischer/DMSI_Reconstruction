...
##### test ende ####
ground_truth_val, ground_truth_train, ground_truth_test = Ground_Truth[..., 0], Ground_Truth[...,1:5].transpose(inverse_reshape), Ground_Truth[...,5]  # Method: Leave last MRSI measurement as test set
Val_Mask, Train_Mask, Test_Mask = mask_extended[...,0], mask_extended[...,1:5].transpose(inverse_reshape), mask_extended[...,5]

#### Assign undersampled network input ####
NN_input_val, NN_input_train, NN_input_test = Undersampled_Data[...,0], Undersampled_Data[...,1:5].transpose(inverse_reshape), Undersampled_Data[...,5]
...
#### reshape for pytorch ####
val_data, val_labels = reshape_for_pytorch(normalized_input_val, grouped_time_steps), reshape_for_pytorch(normalized_ground_truth_val, grouped_time_steps)
...
#### generate random walk training set, first change real and imaginary parts to channel
ground_truth_train = np.stack((np.real(ground_truth_train), np.imag(ground_truth_train)), axis=5)
Train_Mask = np.stack((np.real(Train_Mask), np.imag(Train_Mask)), axis=5)
NN_input_train = np.stack((np.real(NN_input_train), np.imag(NN_input_train)), axis=5)

### generate new data sets and masks
line_data, line_label, line_mask = generate_fixed_random_lines(NN_input_train, ground_truth_train, Train_Mask, num_trajectories=2000, line_length=24)
train_dataset = TensorDataset(line_data, line_label, line_mask)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)


### next, generate random data set 
