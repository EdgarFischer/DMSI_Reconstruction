...
def transform_6d_data_labels_mask(data_6d, labels_6d, mask_6d):
    """
    Apply identical random spatial transformations (shift and rotation) to:
      1) data_6d (complex) of shape (X_d, Y_d, Z_d, t, T, D)
      2) labels_6d (real) of shape (X_l, Y_l, Z_l, t, T, D)
      3) mask_6d (real/binary) of shape (X_l, Y_l, Z_l, t, T, D)
    
    Note:
      - Global phase is applied ONLY to data_6d.
      - Magnitude scaling is applied to both data_6d and labels_6d.
      - The spatial dimensions for data versus labels/mask can differ.
    """
    # Extract shapes; assume t, T, D are the same for data and labels/mask.
    X_d, Y_d, Z_d, t, T, D = data_6d.shape
    X_l, Y_l, Z_l, t_l, T_l, D_l = labels_6d.shape
    ...
    for d in range(D):
        # Extract the d-th slice along the last dimension.
        data_slice   = data_6d[..., d].copy()    # shape: (X_d, Y_d, Z_d, t, T), complex
        labels_slice = labels_6d[..., d].copy()    # shape: (X_l, Y_l, Z_l, t, T), real
        mask_slice   = mask_6d[..., d].copy()      # shape: (X_l, Y_l, Z_l, t, T), real/binary
        
        # Flatten the time dimensions so that we work slice-by-slice in 3D.
        data_slice   = data_slice.reshape(data_slice.shape[:3] + (-1,))    # (X_d, Y_d, Z_d, t*T)
        labels_slice = labels_slice.reshape(labels_slice.shape[:3] + (-1,))  # (X_l, Y_l, Z_l, t*T)
        mask_slice   = mask_slice.reshape(mask_slice.shape[:3] + (-1,))      # (X_l, Y_l, Z_l, t*T)
        
        # 1) Apply a small random shift in x,y.
        ...
        # For labels and mask, use the affine transform for real data.
        for time_idx in range(labels_slice.shape[-1]):
            labels_slice[..., time_idx] = affine_transform_3d_mask(
        ...
        # 3) Apply global phase and magnitude scaling.
        # Global phase is applied ONLY to data.
        ...
        # Apply magnitude scaling to labels.
        for time_idx in range(labels_slice.shape[-1]):
            labels_slice[..., time_idx] = random_scale_magnitude(labels_slice[..., time_idx], scale_factor)
        
        # Reshape back to original dimensions (X, Y, Z, t, T)
        data_slice   = data_slice.reshape(X_d, Y_d, Z_d, t, T)
        labels_slice = labels_slice.reshape(X_l, Y_l, Z_l, 160, T)
        mask_slice   = mask_slice.reshape(X_l, Y_l, Z_l, 160, T)
        
        # Store the transformed slices.
