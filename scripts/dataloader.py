...
class TensorDataset(Dataset):
    def __init__(self, data, labels):
        ...
        Parameters:
        - data: numpy array of shape (N, 2, 22, 22, 21) for input data.
        - labels: numpy array of shape (N, 2, 22, 22, 21) for ground truth labels.
        """
    ...
    def __getitem__(self, idx):
        ...
        Returns:
        - data: PyTorch tensor of shape (2, 22, 22, 21).
        - label: PyTorch tensor of shape (2, 22, 22, 21).
        """
        data = self.data[idx]  # Shape: (2, 22, 22, 21)
        label = self.labels[idx]  # Shape: (2, 22, 22, 21)
        return data, label
    
def generate_model(n, k=1):
    class Net_ver_0_1(nn.Module):
        def __init__(self, n, k):
            super(Net_ver_0_1, self).__init__()
            
            # Convolutional layers for image enhancement
            self.conv1 = nn.Conv3d(k * 2, 64, kernel_size=3, stride=1, padding="same")
            self.conv2 = nn.Conv3d(64, 64, kernel_size=3, stride=1, padding="same")
            self.deconv = nn.ConvTranspose3d(64, 2 * k, kernel_size=3, stride=1, padding=1, output_padding=0)
        ...
        def forward(self, dat):
            ...
            C2 = self.relu(self.conv2(x))  # Keep C2 for regularization if needed
