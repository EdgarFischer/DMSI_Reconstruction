...
for i = 1:numRings
    ...
    % Precompute the average over the first index (1 to 526) for all k values.
    % This will return an array of size [currentKz x ...] that we squeeze to a vector.
    avgData = squeeze(mean(Data{i}(:, 1, :, 1, 2, 1, 8), 1));
...
title('2D Plot of average |signal|');
