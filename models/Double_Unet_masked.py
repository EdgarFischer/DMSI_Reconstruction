...
def double_conv_3d(in_channels, out_channels, use_batch_norm=True):
    """
    Creates two consecutive 3D Conv layers with optional BatchNorm and LeakyReLU.
    """
    layers = []
...
class UNet3D(nn.Module):
    """
    A 3D Double U-Net (YNet) that takes 4-channel input:
      - First 2*k channels: original spectral data (real + imaginary per time step)
      - Next 2 channels: additional masks (e.g., gray matter and white matter masks)
    The network outputs a tensor with 2*k channels (the reconstructed spectral data).
    
    There are two cascaded U-Net stages:
      1. The first stage has a residual connection from the input spectral data 
         to its final output.
      2. The second stage takes as input the first stage’s output concatenated with the original masks,
         and also includes a residual connection from its input spectral data to its final output.
         
    All regular skip connections (from encoder to decoder) are maintained in both stages.
    """
    def __init__(self, grouped_time_steps=1, use_batch_norm=False):
        """
        Args:
            grouped_time_steps (int): Number of time steps to group.
        ...
        # For both U-Nets, input channels: spectral data (2*k) + 2 extra channels (e.g., GM and WM masks)
        in_channels = grouped_time_steps * 2 + 2  
        # Output channels: reconstruct the spectral data (2*k channels)
        out_channels = grouped_time_steps * 2
        ...
        # Final convolution to produce output channels (spectral data)
        self.final_conv = nn.Conv3d(64, out_channels, kernel_size=1)
        
        # ----------------- Second U-Net -----------------
        # For the second stage, we form an input by concatenating:
        #   - The first U-Net’s output (spectral data: 2*k channels)
        #   - The original masks (2 channels)
        # So the second stage also has in_channels = 2*k + 2.
        
        # Encoder (Downsampling)
        self.enc1_2 = double_conv_3d(in_channels, 64, use_batch_norm)
        self.enc2_2 = double_conv_3d(64, 128, use_batch_norm)
        self.enc3_2 = double_conv_3d(128, 256, use_batch_norm)
        self.pool_2 = nn.MaxPool3d(kernel_size=2, stride=2)
        
        # Bottleneck
        self.bottleneck_2 = double_conv_3d(256, 512, use_batch_norm)
        
        # Decoder (Upsampling)
        self.up3_2 = nn.ConvTranspose3d(512, 256, kernel_size=2, stride=2)
        self.dec3_2 = double_conv_3d(256 + 256, 256, use_batch_norm)
        
        self.up2_2 = nn.ConvTranspose3d(256, 128, kernel_size=2, stride=2)
        self.dec2_2 = double_conv_3d(128 + 128, 128, use_batch_norm)
        
        self.up1_2 = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)
        self.dec1_2 = double_conv_3d(64 + 64, 64, use_batch_norm)
        
        # Final convolution to produce output channels (spectral data)
        self.final_conv_2 = nn.Conv3d(64, out_channels, kernel_size=1)
    ...
    def forward(self, dat):
        ...
        Args:
            dat (torch.Tensor): Input of shape (N, 2*k+2, H, W, D), where:
                - The first 2*k channels are spectral data.
                - The last 2 channels are additional masks.
        Returns:
            tuple(torch.Tensor, torch.Tensor): (output, C2)
                - output: Reconstructed spectral data (from the second stage) of shape (N, 2*k, H, W, D),
                          with residual addition.
                - C2: Intermediate feature map from the last decoder stage of the second U-Net.
        """
        # ----------------- First U-Net Stage -----------------
        # Save the residual (original spectral data) from the first 2*k channels
        residual = dat[:, :self.k*2, ...]
        x = dat
        ...
        # Decoder
        u3 = self.up3(b)              # (N, 256, H/4, W/4, D/4)
        cat3 = torch.cat([u3, e3], dim=1)  # (N, 256+256, H/4, W/4, D/4)
        d3 = self.dec3(cat3)          # (N, 256, H/4, W/4, D/4)
        
        u2 = self.up2(d3)             # (N, 128, H/2, W/2, D/2)
        cat2 = torch.cat([u2, e2], dim=1)  # (N, 128+128, H/2, W/2, D/2)
        d2 = self.dec2(cat2)          # (N, 128, H/2, W/2, D/2)
        
        u1 = self.up1(d2)             # (N, 64, H, W, D)
        cat1 = torch.cat([u1, e1], dim=1)  # (N, 64+64, H, W, D)
        # Intermediate decoder output from stage 1 (C2) for the first U-Net
        C2_stage1 = self.dec1(cat1)          # (N, 64, H, W, D)
        
        # Final conv & residual connection for first U-Net stage
        x_out_stage1 = self.final_conv(C2_stage1)   # (N, 2*k, H, W, D)
        x_out_stage1 = x_out_stage1 + residual      # add skip connection from input
        
        # ----------------- Second U-Net Stage -----------------
        # For the second stage, we form a new input by taking:
        #   - x_out_stage1 (reconstructed spectral data: 2*k channels)
        #   - Original masks (last 2 channels of dat)
        second_input = torch.cat([x_out_stage1, dat[:, -2:, ...]], dim=1)
        # Save the residual for the second stage (from its spectral part)
        residual2 = second_input[:, :self.k*2, ...]
        
        # Encoder for second stage
        e1_2 = self.enc1_2(second_input)       # (N, 64, ...)
        p1_2 = self.pool_2(e1_2)
        
        e2_2 = self.enc2_2(p1_2)      # (N, 128, ...)
        p2_2 = self.pool_2(e2_2)
        
        e3_2 = self.enc3_2(p2_2)      # (N, 256, ...)
        p3_2 = self.pool_2(e3_2)
        
        # Bottleneck for second stage
        b_2 = self.bottleneck_2(p3_2) # (N, 512, ...)
        
        # Decoder for second stage
        u3_2 = self.up3_2(b_2)              # (N, 256, H/4, W/4, D/4)
        cat3_2 = torch.cat([u3_2, e3_2], dim=1)  # (N, 256+256, H/4, W/4, D/4)
        d3_2 = self.dec3_2(cat3_2)          # (N, 256, H/4, W/4, D/4)
        
        u2_2 = self.up2_2(d3_2)             # (N, 128, H/2, W/2, D/2)
        cat2_2 = torch.cat([u2_2, e2_2], dim=1)  # (N, 128+128, H/2, W/2, D/2)
        d2_2 = self.dec2_2(cat2_2)          # (N, 128, H/2, W/2, D/2)
        
        u1_2 = self.up1_2(d2_2)             # (N, 64, H, W, D)
        cat1_2 = torch.cat([u1_2, e1_2], dim=1)  # (N, 64+64, H, W, D)
        # Intermediate decoder output from stage 2 (C2)
        C2 = self.dec1_2(cat1_2)            # (N, 64, H, W, D)
        
        # Final conv & residual connection for second stage
        x_out = self.final_conv_2(C2)       # (N, 2*k, H, W, D)
        x_out = x_out + residual2          # add skip connection from the second and third stage input
...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for dat_in, dat_out, mask, gm_mask, wm_mask in data_loader:
        ...
        predictions, C2 = model(inputs)  # Forward pass
...
def validate_model(model, loss_fn, data_loader, device='cpu'):
    ...
    with torch.no_grad():
        for dat_in, dat_out, mask, gm_mask, wm_mask in data_loader:
            ...
            # Forward pass
            predictions, C2 = model(inputs)  # (N, 2*k, H, W, D)
