...
def generate_model(n, k=1):
    class Net_ver_0_0(nn.Module):
        def __init__(self, n, k):
            super(Net_ver_0_0, self).__init__()
            
            # Now we have k time steps, each with 2 channels (real+imag), flattened over n*n pixels
            Nin_fc = 2 * k * n * n  # Input layer size
            # The hidden layers can stay the same if desired, or be adjusted. Here we keep them:
            Nhid = n * n
            
            self.fc1 = nn.Linear(Nin_fc, n * n)
            self.fc2 = nn.Linear(n * n, n * n)
            self.fc3 = nn.Linear(n * n, n * n)
            
            # Convolutional layers remain the same size, but the final layer will now produce 2*k channels.
        ...
        def forward(self, dat):
            ...
            x = self.fc3(x)  # shape: (batch_size, n*n)
            
            # Reshape to (batch_size, 1, n, n) for convolutional layers
            x = x.view(-1, 1, n, n)
            
            x = self.relu(self.conv1(x))
            C2 = self.relu(self.conv2(x))  # Keep C2 for regularization (as in original code)
            x = self.deconv(C2)  # Now produces (batch_size, 2*k, n, n)
            
            return x, C2

    model = Net_ver_0_0(n, k)
    return model
...
def reshape_data(fourier_data, other_data):
    """
    Reshape complex-valued Fourier and image data into a form suitable for training a NN.
    
    Assumptions:
    ------------
    - Input shapes:
        fourier_data: (X, Y, k, N) complex
        other_data:   (X, Y, k, N) complex
      where X,Y are spatial dims, k is the number of grouped time steps, and N is the total samples.
    
    - The goal:
        Fourier data -> (N, X*Y*2*k)
        Other data   -> (N, X, Y, 2*k)

      '2' comes from splitting real and imaginary parts into separate channels.
      
    Parameters
    ----------
    fourier_data : np.ndarray (complex)
        Complex-valued Fourier space data, shape (X,Y,k,N).
    other_data : np.ndarray (complex)
        Complex-valued image (or label) data, shape (X,Y,k,N).

    Returns
    -------
    fourier_final : np.ndarray (real)
        Shaped as (N, X*Y*2*k).
    other_final : np.ndarray (real)
        Shaped as (N, X, Y, 2*k).
    """

    # Check shapes
    if fourier_data.shape != other_data.shape:
        raise ValueError("fourier_data and other_data must have the same shape.")
    
    dims = len(fourier_data.shape)
    if dims == 4:
        # If shape is (X, Y, k, N), extract as expected
        X, Y, k, N = fourier_data.shape
    elif dims == 3:
        # If shape is (X, Y, N) for k=1, add k=1 explicitly
        X, Y, N = fourier_data.shape
        k = 1

    if k == 1:
        # For k = 1, reshape to (X, Y, 1, N, 2) before moving axes
        fourier_ri = np.stack([fourier_data.real, fourier_data.imag], axis=-1)  # (X, Y, N, 2)
        other_ri = np.stack([other_data.real, other_data.imag], axis=-1)        # (X, Y, N, 2)
        
        # Add the k dimension explicitly to make it (X, Y, k, N, 2)
        fourier_ri = np.expand_dims(fourier_ri, axis=2)  # (X, Y, 1, N, 2)
        other_ri = np.expand_dims(other_ri, axis=2)      # (X, Y, 1, N, 2)
    else:
        # For k > 1, split directly
        fourier_ri = np.stack([fourier_data.real, fourier_data.imag], axis=-1)  # (X, Y, k, N, 2)
        other_ri = np.stack([other_data.real, other_data.imag], axis=-1)        # (X, Y, k, N, 2)

    # Move the sample dimension (N) to the front for both arrays.
    # After this, shapes become:
    # fourier_ri: (N,X,Y,k,2)
    # other_ri:   (N,X,Y,k,2)

    fourier_ri = np.moveaxis(fourier_ri, 3, 0)  # Move N from last to first
    other_ri = np.moveaxis(other_ri, 3, 0)      # Move N from last to first

    # Now we have:
    # fourier_ri shape: (N,X,Y,k,2)
    # other_ri shape:   (N,X,Y,k,2)

    # Flatten the fourier_ri spatial and channel dims into a single feature vector per sample:
    # (N,X,Y,k,2) -> (N, X*Y*k*2)
    fourier_final = fourier_ri.reshape(N, X*Y*k*2)

    # Reshape the other_ri to (N,X,Y,2*k):
    # (N,X,Y,k,2) -> (N,X,Y,2*k)
    other_final = other_ri.reshape(N, X, Y, 2*k)

    return fourier_final, other_final
