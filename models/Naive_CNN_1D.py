...
class TensorDataset(Dataset):
    def __init__(self, data, labels):
        ...
        Assumes:
          - data and labels are numpy arrays of shape (N, 2, 8)
            where 2 = real & imaginary, 8 = time steps, and the spatial/frequency dimensions have been collapsed.
        """
    ...
    def __getitem__(self, idx):
        # Each sample is of shape (2, 8)
        data = self.data[idx]   # Shape: (2, 8)
        label = self.labels[idx]  # Shape: (2, 8)
        return data, label

class Naive_CNN_1D(nn.Module):
    """
    A simple 1D convolutional neural network that operates along the time dimension.
    
    The network expects input of shape (batch, in_channels, T),
    where in_channels is the number of input channels (e.g. 2 in this proof-of-principle)
    and T is the number of time steps (8).
    
    The network uses a series of Conv1d layers with a residual connection.
    """
    def __init__(self, in_channels=2, num_convs=3, kernel_size=3):
        """
        Args:
            in_channels (int): The number of input channels. In your current setup, this is 2.
            num_convs (int): Number of convolutional layers (including the final one).
            kernel_size (int): Size of the 1D convolutional kernel (applied along the time dimension).
        """
        super(Naive_CNN_1D, self).__init__()
        
        layers = []
        # First layer: from in_channels to 64 channels.
        layers.append(nn.Conv1d(in_channels, 64, kernel_size=kernel_size, stride=1, padding=kernel_size//2))
        layers.append(nn.LeakyReLU())
        
        # Add (num_convs - 2) intermediate conv layers.
        for _ in range(num_convs - 2):
            layers.append(nn.Conv1d(64, 64, kernel_size=kernel_size, stride=1, padding=kernel_size//2))
            layers.append(nn.LeakyReLU())
            
        self.conv_layers = nn.Sequential(*layers)
        
        # Final convolution to bring the number of channels back to in_channels.
        self.final_conv = nn.Conv1d(64, in_channels, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
        
        # Optional final activation (currently commented out in forward)
    ...
    def forward(self, x):
        ...
        Input:
            x: Tensor of shape (batch, in_channels, T) where T is the number of time steps (8).
        Processing:
            1. Applies several Conv1d layers along the time dimension.
            2. Uses a residual connection to add the input back at the end.
        Returns:
            A tuple (out, C2) where:
                - out: Final output tensor of shape (batch, in_channels, T)
                - C2: The intermediate feature map from the conv layers.
        """
        # x is already of shape (B, in_channels, T)
        out = self.conv_layers(x)
        C2 = out  # Save intermediate features if needed.
        
        # Add a residual connection from the input.
        out = self.final_conv(C2) + x
        
        # Optionally, uncomment the following line to apply a final activation:
...
def validate_model(model, loss_fn, data_loader, device='cpu'):
    """
    Validation loop to compute average loss (MSE) only.
    """
