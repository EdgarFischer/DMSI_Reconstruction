...
class TensorDatasetWithAugmentation(torch.utils.data.Dataset):
    def __init__(self, data, labels, norm_values, transform=None):
        """
        Args:
...
def double_conv_2d(in_channels, out_channels, use_batch_norm=True):
    """
    A helper function to create two consecutive 2D Conv + (optional) BatchNorm + LeakyReLU layers.
    """
    layers = []
...
class UNet2D(nn.Module):
    """
    A 2D U-Net for image enhancement, without any masking logic.
    ...
    def __init__(self, grouped_time_steps=1, use_batch_norm=False):
        ...
        self.enc1 = double_conv_2d(in_channels, 64, use_batch_norm)
        self.enc2 = double_conv_2d(64, 128, use_batch_norm)
        self.enc3 = double_conv_2d(128, 256, use_batch_norm)
        
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # ----------- Bottleneck -----------
        self.bottleneck = double_conv_2d(256, 512, use_batch_norm)
        
        # ----------- Decoder (Upsampling) -----------
        self.up3 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2)
        self.dec3 = double_conv_2d(256 + 256, 256, use_batch_norm)
        
        self.up2 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec2 = double_conv_2d(128 + 128, 128, use_batch_norm)
        
        self.up1 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.dec1 = double_conv_2d(64 + 64, 64, use_batch_norm)
        
        # Final 2D conv to get back to "out_channels"
        self.final_conv = nn.Conv2d(64, out_channels, kernel_size=1)

    def forward(self, dat):
        """
        Forward pass of the 2D U-Net, ignoring any masking logic.
        
        Args:
            dat (torch.Tensor): Input of shape (batch_size, 2*k, H, W).
        
        Returns:
            tuple(torch.Tensor, torch.Tensor): (output, C2)
                - output is the final enhanced image with shape (batch_size, 2*k, H, W)
                - C2 is an intermediate feature map (useful for regularization or inspection)
        """
        x = dat
        ...
        # ---------------- Decoder ----------------
        u3 = self.up3(b)              # upsample -> (batch, 256, H/4, W/4)
        cat3 = torch.cat([u3, e3], 1)  # skip connection
        d3 = self.dec3(cat3)          # (batch, 256, H/4, W/4)
        
        u2 = self.up2(d3)             # upsample -> (batch, 128, H/2, W/2)
        cat2 = torch.cat([u2, e2], 1)  # skip connection
        d2 = self.dec2(cat2)          # (batch, 128, H/2, W/2)
        
        u1 = self.up1(d2)             # upsample -> (batch, 64, H, W)
        cat1 = torch.cat([u1, e1], 1)  # skip connection
        C2 = self.dec1(cat1)          # (batch, 64, H, W)
        
        # ---------------- Final Conv & Residual ----------------
        x = self.final_conv(C2)       # (batch, 2*k, H, W)
        
        # Residual addition (can remove this if not desired)
        x = x + dat
...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for dat_in, dat_out, norm_value in data_loader:
...
def validate_model(
    ...
    with torch.no_grad():
        for dat_in, dat_out, mask, norm_value in data_loader:
            # Move input, output, mask, and norm_value to the specified device
            ...
            # These indices reverse the padding applied (D:1:-1, H:1:-1, W:1:-2)
            ...
            masked_predictions = predictions_cropped * mask_cropped * norm_value
            masked_targets     = targets_cropped * mask_cropped * norm_value
