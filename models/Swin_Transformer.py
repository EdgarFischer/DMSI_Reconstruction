...
def window_partition_3d(x, window_size):
    """
    Partition input x (B, C, D, H, W) into 3D windows of size window_size^3.
    Returns windows of shape (num_windows * B, C, window_size, window_size, window_size)
    and also the original (D, H, W) for reversing the process.
    """
    B, C, D, H, W = x.shape
    d_splits = D // window_size
    h_splits = H // window_size
    w_splits = W // window_size

    # (B, C, d_splits, window_size, h_splits, window_size, w_splits, window_size)
    x = x.view(B, C, d_splits, window_size, h_splits, window_size, w_splits, window_size)
    # rearrange so windows form the batch dimension
    x = x.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous()  # (B, d_splits, h_splits, w_splits, C, win, win, win)
    windows = x.view(-1, C, window_size, window_size, window_size)
    return windows, (d_splits, h_splits, w_splits)
...
def window_reverse_3d(windows, window_size, B, C, D, H, W):
    ...
    h_splits = H // window_size
    w_splits = W // window_size
    num_windows_per_dim = d_splits * h_splits * w_splits

    x = windows.view(B, d_splits, h_splits, w_splits, C, window_size, window_size, window_size)
    x = x.permute(0, 4, 1, 5, 2, 6, 3, 7).contiguous()
    x = x.view(B, C, D, H, W)
    return x


def compute_mask_3d(D, H, W, window_size, shift_size):
    """
    Precompute attention mask to handle the 'shifted windows' so that
    out-of-window connections are masked properly.
    """
    # We'll create an indices map to assign each voxel an index for the (d,h,w) partition
    img_mask = torch.zeros((1, 1, D, H, W))  # (B=1, C=1) just for indexing
    ...
    # windows = window_partition_3d(img_mask, window_size)
    # But for simplicity, let's just do one mask for shift.
    # Usually we combine the mask logic for parted & shifted windows.

    # This is a toy example – real Swin code computes multiple masks for parted/shifted windows.
    return img_mask
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x):
        """
        x: (B*num_windows, win_d*win_h*win_w, dim)
        """
        Bnw, N, C = x.shape
...
class SwinTransformerBlock3D(nn.Module):
    """
    One Swin Transformer block with window-based attention, optionally shifted.
    """
    ...
    def forward(self, x, B, D, H, W):
        """
        x: (B, C=dim, D, H, W)
        We apply LN -> W-MSA (windowed attention) -> skip -> LN -> MLP -> skip
        """
        shortcut = x
        ...
        windows, (d_splits, h_splits, w_splits) = window_partition_3d(x, self.window_size)
        # windows shape -> (B*num_windows, C, wd, wh, ww)

        # Flatten spatial dims for attention
        Bnw, C_, wd, wh, ww = windows.shape
        windows = windows.view(Bnw, C_, wd*wh*ww).transpose(1,2)  # (Bnw, wd*wh*ww, C_)

        # W-MSA
        attn_windows = self.attn(windows)  # (Bnw, wd*wh*ww, C_)

        # Reverse reshape
        attn_windows = attn_windows.transpose(1,2).view(Bnw, C_, wd, wh, ww)
        x = window_reverse_3d(attn_windows, self.window_size, B, self.dim, D, H, W)
        ...
        # MLP
        shortcut2 = x
        x = self.norm2(x.permute(0,2,3,4,1))  # (B, D, H, W, C)
        x = x.view(B*D*H*W, self.dim)
        x = self.mlp(x)
        x = x.view(B, D, H, W, self.dim)
...
class BasicLayer3D(nn.Module):
    """
    A 'stage' of Swin: potentially multiple SwinTransformerBlocks with optional patch merging.
    """
    def __init__(self, dim, depth, num_heads, window_size, shift_sizes):
        super().__init__()
        self.blocks = nn.ModuleList([
            SwinTransformerBlock3D(dim, num_heads, window_size, shift_size=shift_sizes[i])
...
class SwinTransformer3D(nn.Module):
    """
    Minimal 3D Swin 'U-like' model that keeps input resolution (no real down/up sampling).
    Outputs the same shape as input for a potential 'reconstruction' use case.
    """
    def __init__(self, 
                 in_channels=2,   # real + imaginary
                 embed_dim=64,    
                 window_size=4,   
                 depths=(2, 2),   # number of blocks in each stage
                 num_heads=(2, 2),
                 shift_sizes=((0,2),(0,2)),  # shift pattern per stage, or per block
                 ):
        super().__init__()
        ...
        # 1) Initial projection (Conv or linear patch embed)
        self.patch_embed = nn.Conv3d(in_channels, embed_dim, kernel_size=3, padding=1)

        # 2) Stages of Swin blocks (here we use two stages for demonstration)
        #    You could add patch merging for a hierarchical approach, but we skip that
        #    to preserve the same input-output resolution in this minimal example.
        self.stage1 = BasicLayer3D(dim=embed_dim,
                                   depth=depths[0],
                                   num_heads=num_heads[0],
                                   window_size=window_size,
                                   shift_sizes=shift_sizes[0])
        
        self.stage2 = BasicLayer3D(dim=embed_dim,
                                   depth=depths[1],
                                   num_heads=num_heads[1],
                                   window_size=window_size,
                                   shift_sizes=shift_sizes[1])
        
        # 3) Final projection back to 2 channels
        self.final_conv = nn.Conv3d(embed_dim, in_channels, kernel_size=1)
        
    def forward(self, x):
        """
        x: (B, 2, D, H, W)
        returns shape (B, 2, D, H, W)
        """
        B, C, D, H, W = x.shape
        # Patch Embedding
        x_embed = self.patch_embed(x)  # (B, embed_dim, D, H, W)

        # Stage 1
        x_stage1 = self.stage1(x_embed, B, D, H, W)
        # Stage 2
        x_stage2 = self.stage2(x_stage1, B, D, H, W)

        # Final projection
        out = self.final_conv(x_stage2)+x
        
        # Optionally, add a residual skip
        # out = out + x  # If you want a direct residual from input
        return out
...
#         in_channels=2,
#         embed_dim=64,      # You can play with this
#         window_size=4,     # Must divide the input D,H,W
#         depths=(2, 2),     # 2 blocks in stage1, 2 blocks in stage2
#         num_heads=(2, 2),  # number of attention heads per stage
#         # shift_sizes=((0,2),(0,2)) means the first block in a stage has shift=0, 
#         # the second block has shift=2 => "shifted windows"
#         shift_sizes=((0,2),(0,2))
#     ).cuda()

#     # Dummy input
#     B, C, D, H, W = 1, 2, 32, 32, 32  # for example
#     x = torch.randn(B, C, D, H, W).cuda()

#     y = model(x)
#     print("Output shape:", y.shape)  # Should be (1, 2, 32, 32, 32)
...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for dat_in, dat_out, mask in data_loader:
        ...
        # Forward pass
        predictions = model(dat_in)
        
        # --- CROP TO ORIGINAL SIZE IF NEEDED ---
        predictions_cropped = predictions  # e.g., predictions[:, :, 1:-1, 1:-1, 1:-2]
        targets_cropped = dat_out         # e.g., dat_out[:, :, 1:-1, 1:-1, 1:-2]
        mask_cropped = mask               # e.g., mask[:, :, 1:-1, 1:-1, 1:-2]
...
####################################
# Validation loop (no C2 references)#
####################################
