...
def double_conv_3d(in_channels, out_channels, use_batch_norm=False):
    """
    Performs two 3D convolutions with optional batch norm.
    """
    if use_batch_norm:
...
class Encoder3D(nn.Module):
    """
    Simple 3D Encoder with 3 levels + bottleneck for demonstration.
    """
    def __init__(self, in_channels=2, use_batch_norm=False):
        super().__init__()
...
class WNet3D(nn.Module):
    """
    A 3-branch 3D U-Net-like architecture that fuses three encoders into one decoder.
    """
    def __init__(self, 
                 in_channels_branchA=2,
                 in_channels_branchB=2,
                 in_channels_branchC=2,
                 use_batch_norm=False):
        """
        Args:
            in_channels_branchA, in_channels_branchB, in_channels_branchC:
                Number of channels in each input branch (e.g., real+imag for a spectral window).
        ...
        self.encC = Encoder3D(in_channels_branchC, use_batch_norm)
        
        # --------------- Decoder ---------------
        # Because we fuse 3 bottlenecks, the combined channels for the bottleneck is 512 * 3
        self.dec_channels_in = 512 * 3
        
        # You might do a small "fuse_conv" on these fused channels before actual upsampling
        self.bottleneck_fuse = double_conv_3d(self.dec_channels_in, 512, use_batch_norm)
        
        # Now define upsampling / decoding layers. 
        # We'll assume we have a standard 3-level decoder matching the original U-Net sizes:
        
        self.up3 = nn.ConvTranspose3d(512, 256, kernel_size=2, stride=2)
        self.dec3 = double_conv_3d(256 + 256*3, 256, use_batch_norm)  
        # note: we cat e3 from each branch => 3*256 + 256 from upsample = 256+768 total in
        
        self.up2 = nn.ConvTranspose3d(256, 128, kernel_size=2, stride=2)
        self.dec2 = double_conv_3d(128 + 128*3, 128, use_batch_norm)
        
        self.up1 = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)
        self.dec1 = double_conv_3d(64 + 64*3, 64, use_batch_norm)
        
        # Final output channels: let's assume we want 2 (real+imag) or whatever
        self.final_conv = nn.Conv3d(64, 2, kernel_size=1)
        
    def forward(self, xA, xB, xC):
        """
        Args:
            xA: (N, in_channels_branchA, X, Y, Z)
            xB: (N, in_channels_branchB, X, Y, Z)
            xC: (N, in_channels_branchC, X, Y, Z)
            
        Returns:
            out: (N, 2, X, Y, Z) final fused volume
        """
        
        # ---------------- Encoder Passes ----------------
        e1A, e2A, e3A, bA = self.encA(xA)
        e1B, e2B, e3B, bB = self.encB(xB)
        e1C, e2C, e3C, bC = self.encC(xC)
        
        # ---------------- Fuse Bottlenecks ----------------
        # Each bottleneck is (N, 512, X/8, Y/8, Z/8), so we can cat along channels
        bottleneck_cat = torch.cat([bA, bB, bC], dim=1)  # => (N, 512*3, ...)
        
        # Apply a small conv to fuse them down to 512 again
        fused_b = self.bottleneck_fuse(bottleneck_cat)   # => (N, 512, ...)
        
        # ---------------- Decoder: Level 3 ----------------
        # Upsample fused bottleneck
        u3 = self.up3(fused_b)   # => (N, 256, X/4, Y/4, Z/4)
        
        # Combine skip connections from e3A, e3B, e3C (each is (N,256,...))
        cat3 = torch.cat([u3, e3A, e3B, e3C], dim=1)  # => (N, 256 + 256*3, ...)
        d3 = self.dec3(cat3) # => (N, 256, ...)
        
        # ---------------- Decoder: Level 2 ----------------
        u2 = self.up2(d3)   # => (N,128, X/2, Y/2, Z/2)
        cat2 = torch.cat([u2, e2A, e2B, e2C], dim=1)  # => (N,128+128*3, ...)
        d2 = self.dec2(cat2) # => (N,128, ...)
        
        # ---------------- Decoder: Level 1 ----------------
        u1 = self.up1(d2)   # => (N,64, X, Y, Z)
        cat1 = torch.cat([u1, e1A, e1B, e1C], dim=1) # => (N,64+64*3, ...)
        d1 = self.dec1(cat1) # => (N,64, ...)
...
def train_one_epoch_wnet(model, optimizer, loss_fn, data_loader, device="cuda"):
    model.train()
    epoch_loss = 0.0
    for batch_idx, (inputs, targets, masks) in enumerate(data_loader):
        # inputs shape: (batch_size, 2, z, f, T)
        inputs = inputs.to(device)   # (N,2,z,f,T)
        targets = targets.to(device) # e.g. (N,2,z,f,T)
        
        # Split into 3 branches
        xA, xB, xC = split_spectrum_into_3(inputs,
                                           idxA=(0,55),
                                           idxB=(56,71),
                                           idxC=(72,None))
        ...
        # Forward pass
        preds = model(xA, xB, xC)  # -> (N,2,z,f,T)
...
def validate_model(model, loss_fn, data_loader, device='cpu'):
    """
    Validation loop to compute average loss on a multi-branch (W‑Net) model 
    by splitting the input spectrum into separate branches.
    
    Args:
        model (nn.Module): The WNet3D (or similar) model with three encoder branches.
        loss_fn (callable): A loss function taking (predictions, targets) or
                            possibly including additional arguments if needed.
        data_loader (DataLoader): Yields batches of (dat_in, dat_out, mask).
            - dat_in: (batch_size, 2, z, f, T)  (2=real+imag, z=spatial, f=frequency, T=temporal)
            - dat_out: same shape as dat_in, representing the ground-truth or target
            - mask: same shape for specifying valid regions
    ...
    with torch.no_grad():
        for dat_in, dat_out, mask in data_loader:
            # ------------------------------------------------------------
            # 1) Move inputs, outputs, and mask to the specified device
            # ------------------------------------------------------------
            dat_in  = dat_in.to(device)    # shape: (N, 2, z, f, T)
            dat_out = dat_out.to(device)   # shape: (N, 2, z, f, T)
            mask    = mask.to(device)      # shape: (N, 2, z, f, T)

            # ------------------------------------------------------------
            # 2) Split the input data into three frequency windows
            #    for the W‑Net's three encoder branches
            # ------------------------------------------------------------
            xA, xB, xC = split_spectrum_into_3(
                dat_in,
                idxA=(0,55),   # frequencies 0..54
                idxB=(56,71),  # frequencies 56..70
                idxC=(72,None) # frequencies 72..end
            )
            # Move the split inputs onto device (they may already be on device,
            # but this is safe to ensure).
            xA, xB, xC = xA.to(device), xB.to(device), xC.to(device)

            # ------------------------------------------------------------
            # 3) Forward pass through the model
            # ------------------------------------------------------------
            # The model is assumed to have signature: model(xA, xB, xC)
            predictions = model(xA, xB, xC)  # shape: (N, 2, z, f, T)

            # ------------------------------------------------------------
            # 4) Apply masking (if you want to compute loss only on valid regions)
            # ------------------------------------------------------------
            # We assume 'mask' is 1.0 in valid regions, 0.0 in invalid ones
            masked_predictions = predictions * mask
            masked_targets     = dat_out    * mask

            # ------------------------------------------------------------
            # 5) Compute loss on the masked data
            # ------------------------------------------------------------
            # Here, your loss_fn might be something like MSE, L1, etc.
            # If your loss_fn signature is just (pred, target), then do:
            loss_curr = loss_fn(masked_predictions, masked_targets)
            
            # If your custom loss function requires extra arguments (like model, features, etc.),
            # you'd call it accordingly, e.g. loss_fn(masked_predictions, masked_targets, model, ...)
    ...
    # ------------------------------------------------------------
    # 6) Compute and return the average loss over the entire dataset
    # ------------------------------------------------------------
