...
class UNet3D(nn.Module):
    """
    A 3D U-Net (YNet) that takes 4-channel input:
    ...
    and includes a residual connection that adds the original spectral data (first 2 channels)
    to the final output.
    """
    def __init__(self, grouped_time_steps=1, use_batch_norm=False):
        ...
        # Input channels: spectral data (2 * k) + 2 extra channels (e.g., GM and WM masks)
    ...
    def forward(self, dat):
        ...
        Args:
            dat (torch.Tensor): Input of shape (N, 2*k+2, H, W, D), where the first 2*k channels
                                are spectral data and the last 2 channels are additional masks.
        Returns:
            tuple(torch.Tensor, torch.Tensor): (output, C2)
                - output: Reconstructed spectral data of shape (N, 2*k, H, W, D) with residual addition.
                - C2: Intermediate feature map from the last decoder stage.
        """
        ...
        # ---------------- Decoder ----------------
        u3 = self.up3(b)              # Upsample to (N, 256, H/4, W/4, D/4)
        cat3 = torch.cat([u3, e3], dim=1)  # Concatenate with encoder level 3: (N, 256+256, H/4, W/4, D/4)
        ...
        C2 = self.dec1(cat1)          # (N, 64, H, W, D)
        
        # ---------------- Final Conv & Residual ----------------
        x_out = self.final_conv(C2)   # (N, 2*k, H, W, D)
        
        # Add the residual from the original spectral data
