...
class MRSIDataset(Dataset):
    def __init__(self, data, labels, masks):
        """
        Parameters:
          data: numpy array expected to be of shape (x, y, z, t, T, D, 2)
          labels: numpy array with the same shape as data.
          masks: list or numpy array of length D, each element is a mask for the patient,
                 e.g., shape (x, y, z) or (x, y, z, 1)
        """
        # Check and adjust the number of dimensions
        if data.ndim == 6:
            # Assume the patient dimension is missing; insert a singleton dimension.
            data = data[..., np.newaxis, :]
            labels = labels[..., np.newaxis, :]

        # Get original spatial and other dimensions
        self.x, self.y, self.z, self.t, self.T, self.D, self.ch = data.shape
        
        # Reshape the data to collapse t and T into a single dimension per patient.
        # New shape: (x, y, z, t*T, D, 2)
        data_reshaped = data.reshape(self.x, self.y, self.z, self.t * self.T, self.D, self.ch)
        labels_reshaped = labels.reshape(self.x, self.y, self.z, self.t * self.T, self.D, self.ch)
        
        # Prepare a list of samples.
        # Each sample is a 3D volume with shape (2, x, y, z) (after moving the channel dim),
        # and we store also the corresponding mask.
        self.samples = []
        for d in range(self.D):
            for time in range(self.t * self.T):
                # Extract a 3D volume for this patient and time point.
                # data_volume has shape (x, y, z, 2) so we need to move the channel to the front.
                data_volume = data_reshaped[:, :, :, time, d, :]  # shape: (x, y, z, 2)
                label_volume = labels_reshaped[:, :, :, time, d, :]  # shape: (x, y, z, 2)
                # Move the channel dimension: (2, x, y, z)
                data_volume = data_volume.transpose(3, 0, 1, 2)
                label_volume = label_volume.transpose(3, 0, 1, 2)
                # Get the corresponding mask for patient d.
                mask = masks[d]
                self.samples.append({
                    'data': torch.tensor(data_volume, dtype=torch.float32),
                    'label': torch.tensor(label_volume, dtype=torch.float32),
...
class Naive_CNN_3D(nn.Module):
    ...
    def __init__(self, grouped_time_steps=1, num_convs=3, use_batch_norm=True):
        ...
        layers = []
        in_channels = grouped_time_steps * 2  # 2 for real + imaginary channels
        for i in range(self.num_convs - 1):  # Last layer is handled separately
            layers.append(
                nn.Conv3d(
                    in_channels,
                    64,
    ...
    def forward(self, dat, mask):
        """
        Forward pass for the Naive_CNN_3D.

        Args:
            dat (torch.Tensor): Input tensor of shape (batch_size, 2*grouped_time_steps, x, y, z)
            mask (torch.Tensor): Mask tensor of shape (batch_size, x, y, z)

        Returns:
            tuple: Output tensor and intermediate features for optional regularization.
        """
        if mask is None:
            raise ValueError("Mask must be provided for each sample")

        # Expand mask to match input shape: (batch_size, 1, x, y, z)
        mask = mask.unsqueeze(1)  # Add channel dim
        mask = mask.repeat(1, dat.size(1), 1, 1, 1)  # Repeat across real/imag channels
        ...
        C2 = x  # Intermediate feature map
        x = self.final_conv(C2) + dat  # Residual connection
...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for batch in data_loader:
        # Extract the input data, target labels, and per-sample masks from the batch dictionary.
        dat_in = batch['data'].to(device)    # Expected shape: (batch, 2, x, y, z)
        dat_out = batch['label'].to(device)    # Expected shape: (batch, 2, x, y, z)
        mask    = batch['mask'].to(device)     # Expected shape: (batch, x, y, z)
        
        optimizer.zero_grad()  # Clear previous gradients
        
        # Forward pass: note that we pass the mask along with the data.
        predictions, C2 = model(dat_in, mask)
        
        # Compute loss; your loss_fn is assumed to accept (predictions, dat_out, model, C2)
        loss_curr = loss_fn(predictions, dat_out, model, C2)
        
        loss_curr.backward()  # Compute gradients
        optimizer.step()      # Update model weights
        
        total_loss += loss_curr.item() * dat_in.size(0)  # Accumulate loss weighted by batch size
        num_samples += dat_in.size(0)
...
def validate_model(
    ...
    Args:
        ...
        loss_fn (callable): Loss function that accepts (predictions, labels, model, C2).
        data_loader (DataLoader): DataLoader returning dictionaries with keys 'data', 'label', and 'mask'.
        device (str): Device to run the validation on.
        psnr_metric (optional): Metric function for PSNR.
        ssim_metric (optional): Metric function for SSIM.

    Returns:
        tuple: (average loss, average PSNR, average SSIM)
    """
    ...
    with torch.no_grad():
        for batch in data_loader:
            # Extract the inputs, labels, and masks from the batch dictionary
            dat_in = batch["data"].to(device)   # Expected shape: (batch, 2*grouped_time_steps, x, y, z)
            dat_out = batch["label"].to(device)   # Expected shape: (batch, 2*grouped_time_steps, x, y, z)
            mask    = batch["mask"].to(device)    # Expected shape: (batch, x, y, z)
