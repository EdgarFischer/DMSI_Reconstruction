...
class RandomAugment3D:
    ...
    Combined 3D augmentation:
      - Random phase multiplication: Each complex channel pair (real, imag) is multiplied by exp(i * phi)
      - Random rotation in the (H,W) plane by an angle in [-rotation_range, rotation_range] (in radians)
      - Random shifting by up to shift_pixels in each spatial dimension.
    
    Assumes input tensors have shape (C, H, W, D) where C is even 
    (with channels 0,1 forming the first complex pair, 2,3 the second, etc.).
    """
    def __init__(self, rotation_range=0.3, shift_pixels=1,
    ...
    def apply_random_rotation(self, data, label, mask):
        """
        Rotate the volume in the (H, W) plane (i.e. the 2nd and 3rd dimensions)
        while leaving the D dimension unchanged.
        """
        angle = random.uniform(-self.rotation_range, self.rotation_range)
        # Construct an affine matrix for rotation in the (W, H) plane.
        # Note: For grid_sample in 3D, input shape should be (N, C, D, H, W)
        # and the affine matrix is 3x4 acting on (x, y, z) coordinates (x corresponds to W, y to H, z to D).
        theta = torch.tensor([
            [math.cos(angle), -math.sin(angle), 0, 0],
            [math.sin(angle),  math.cos(angle), 0, 0],
            [0,                0,               1, 0]
        ], dtype=torch.float)
        theta = theta.unsqueeze(0)  # Shape: (1, 3, 4)

        def rotate_tensor(tensor):
            # tensor has shape (C, H, W, D)
            # Add a batch dimension and permute to shape (N, C, D, H, W)
            tensor = tensor.unsqueeze(0)            # (1, C, H, W, D)
            tensor = tensor.permute(0, 1, 4, 2, 3)     # (1, C, D, H, W)
            N, C, D, H, W = tensor.shape
            # Create the grid for the affine transformation.
            grid = F.affine_grid(theta, size=(N, C, D, H, W), align_corners=True)
            # Apply the affine transformation.
            tensor_rot = F.grid_sample(tensor, grid, mode='bilinear', padding_mode='border', align_corners=True)
            # Permute back to original shape (C, H, W, D) and remove the batch dimension.
            tensor_rot = tensor_rot.permute(0, 1, 3, 4, 2).squeeze(0)
            return tensor_rot
    ...
    def apply_random_shift(self, data, label, mask):
        """
        Shift the volume by a random offset (in pixels) along each spatial dimension.
        """
    ...
    def __call__(self, data, label, mask):
        # data, label, mask are assumed to be tensors of shape (C, H, W, D)
        if self.apply_rotation:
            data, label, mask = self.apply_random_rotation(data, label, mask)
        if self.apply_shift:
...
class TensorDatasetWithAugmentation(torch.utils.data.Dataset):
    def __init__(self, data, labels, masks, transform=None):
        ...
        transform: callable, applied as transform(data, label, mask)
        """
...
class Naive_CNN_3D(nn.Module):
    ...
    def __init__(self, grouped_time_steps=1, num_convs=3, use_batch_norm=True, mask=None):
        ...
        layers = []
        in_channels = grouped_time_steps
        ...
        self.final_conv = nn.Conv3d(
            64, grouped_time_steps, kernel_size=3, stride=1, padding="same"
        )

        # Optional activation function for final output
        self.tanh = nn.LeakyReLU()
        
        # Initialize mask as a 3D tensor with ones if not provided
        if mask is None:
            self.mask = None  # To allow dynamic masking for each batch
        else:
            self.register_buffer('mask', mask)  # Register as a persistent buffer (not trainable)


    def forward(self, dat, mask=None):
        """
        ...
        """
        # Use class-level mask if no mask is provided in the forward method
        effective_mask = mask if mask is not None else self.mask
        if effective_mask is None:
            # If no mask is provided, default to ones (no masking)
            effective_mask = torch.ones_like(dat[:, 0, ...])  # Shape (batch_size, height, width, depth)

        # Apply the mask to both real and imaginary parts (assumes dat has 2 channels per grouped time step)
        effective_mask = effective_mask.unsqueeze(1).repeat(1, dat.size(1), 1, 1, 1)  # Match shape
        dat = dat * effective_mask
        ...
        # Apply the mask to the output as well
        x = x * effective_mask
...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for dat_in, dat_out, mask in data_loader:
        ...
        # Compute loss, now including the mask
...
def validate_model(
    ...
    using masks to focus on valid regions.
    """
    ...
    with torch.no_grad():
        for dat_in, dat_out, mask in data_loader:
            # Move input, output, and mask to the specified device
            dat_in, dat_out, mask = dat_in.to(device), dat_out.to(device), mask.to(device)
            ...
            # Compute and accumulate PSNR if provided
            if psnr_metric is not None:
                psnr_val = psnr_metric(masked_predictions, masked_targets)
                total_psnr += psnr_val.item()

            # Compute and accumulate SSIM if provided
            if ssim_metric is not None:
                ssim_val = ssim_metric(masked_predictions, masked_targets)
