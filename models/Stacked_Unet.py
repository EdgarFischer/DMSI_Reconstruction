...
class UNet3D(nn.Module):
    """
    A 3D U-Net for image enhancement without returning the C2 layer.
    """
    ...
    def forward(self, x):
        ...
        out = out + x  # Residual addition (if desired)
        
        return out


#############################
# Stacked UNet3D definition #
#############################
class StackedUNet3D(nn.Module):
    """
    Stacks N copies of the UNet3D model on top of each other.
    The output of UNet[i] is fed as the input to UNet[i+1].
    """
    def __init__(self, num_unets=2, grouped_time_steps=1, use_batch_norm=False):
        super(StackedUNet3D, self).__init__()
        
        # Create N copies of UNet3D and store them in a ModuleList
        self.unets = nn.ModuleList([
            UNet3D(grouped_time_steps=grouped_time_steps, use_batch_norm=use_batch_norm)
            for _ in range(num_unets)
        ])
        
    def forward(self, x):
        """
        Passes the input x through each UNet in sequence.
        
        Args:
            x (torch.Tensor): Input of shape (B, 2*k, H, W, D).
        
        Returns:
            torch.Tensor: The final output (after passing through all UNets).
        """
