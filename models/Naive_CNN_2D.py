...
class TensorDataset(Dataset):
    def __init__(self, data, labels):
        ...
        Parameters:
        - data: numpy array of shape (N, 2, t, T) for input data.
        - labels: numpy array of shape (N, 2, t, T) for ground truth labels.
        """
        # Convert numpy arrays to PyTorch tensors
        self.data = torch.tensor(data, dtype=torch.float32)  # Shape: (N, 2, t, T)
        self.labels = torch.tensor(labels, dtype=torch.float32)  # Shape: (N, 2, t, T)
    ...
    def __getitem__(self, idx):
        ...
        Returns:
        - data: PyTorch tensor of shape (2, x, y).
        - label: PyTorch tensor of shape (2, x, y).
        """
        data = self.data[idx]  # Shape: (2, x, y)
        label = self.labels[idx]  # Shape: (2, x, y)
        return data, label
    
class Naive_CNN_2D(nn.Module):
    """A customizable 2D convolutional neural network for image enhancement."""
    def __init__(self, in_channels=2, num_convs=3):
        """
        Args:
            in_channels (int): Number of input channels. 
                               Example: 2 if you have real/imag channels.
            num_convs (int): How many convolution layers (excluding the final one).
        """
        super(Naive_CNN_2D, self).__init__()

        self.num_convs = num_convs

        # Create a list of layers that will form our sequential block
        layers = []
        current_channels = in_channels

        # Build the first (num_convs - 1) conv layers
        ...
        # Package these layers into a single sequential module
        self.conv_layers = nn.Sequential(*layers)

        # Final convolution layer maps back to in_channels (for residual addition)
        ...
        # Optional non-linear activation for the final output
        self.leaky_relu = nn.LeakyReLU(inplace=True)

    def forward(self, x):
        """
        Forward pass for the Naive_CNN_2D.
        ...
        Returns:
            tuple:
                - out (torch.Tensor): The enhanced output of shape (batch_size, in_channels, height, width)
                - C2 (torch.Tensor): The intermediate feature map (for optional regularization).
        """
        # Store the initial input for residual connection
        input_data = x

        # Pass through the main convolutional layers
        x = self.conv_layers(x)

        # Capture intermediate features
        C2 = x

        # Final convolution + residual connection
        x = self.final_conv(x) + input_data

        # (Optionally apply a final activation; uncomment if desired)
        # x = self.leaky_relu(x)
