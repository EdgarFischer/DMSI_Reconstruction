...
def train_one_epoch(model, optimizer, loss_fn, data_loader, device='cpu'):
    ...
    for dat_in, dat_out, mask in data_loader:
        dat_in, dat_out, mask = dat_in.to(device), dat_out.to(device), mask.to(device)  # Move to device
        
        optimizer.zero_grad()  # Clear previous gradients
        predictions, C2 = model(dat_in)  # Forward pass
        loss_curr = loss_fn(predictions*mask, dat_out*mask, model, C2)  # Compute loss
