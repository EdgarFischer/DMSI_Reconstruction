    ...
    def forward(self, dat):
        ...
        Returns:
            tuple(torch.Tensor, torch.Tensor): (output, C2)
                - output is the final enhanced volume with shape (batch_size, 2*k, H, W, D)
        ...
        e2 = self.enc2(p1)      # (batch, 128, ...)
        p2 = self.pool(e2)      # downsample
        
        e3 = self.enc3(p2)      # (batch, 256, ...)
        p3 = self.pool(e3)      # downsample
        
        # ---------------- Bottleneck ----------------
        b = self.bottleneck(p3) # (batch, 512, ...)
        
        # ---------------- Decoder ----------------
        u3 = self.up3(b)              # upsample -> (batch, 256, ...)
        cat3 = torch.cat([u3, e3], 1) # skip connection
        d3 = self.dec3(cat3)          # (batch, 256, ...)
        
        u2 = self.up2(d3)             # upsample -> (batch, 128, ...)
        cat2 = torch.cat([u2, e2], 1) # skip connection
        d2 = self.dec2(cat2)          # (batch, 128, ...)
        
        u1 = self.up1(d2)             # upsample -> (batch, 64, ...)
        cat1 = torch.cat([u1, e1], 1) # skip connection
        C2 = self.dec1(cat1)          # (batch, 64, ...)
        
        # ---------------- Final Conv & Residual ----------------
        x = self.final_conv(C2)       # (batch, 2*k, H, W, D)
